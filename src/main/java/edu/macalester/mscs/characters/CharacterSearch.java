...
//                "src/main/resources/text/feastforcrows.txt");
    ...
    private static void updateCharListFile(String charFileName, String suffix, String[] charCatalogNames, String textFileName) {
        ...
        for (String currentLine : charList) {
            String currentKey = StringUtils.substringBefore(currentLine, ",");
            charMap.put(currentKey, "old," + currentLine);
        }

        for (String charCatalogName : charCatalogNames) {
            Map<String, String> tempMap = processFile(textFileName, charCatalogName);
        ...
        for (String temp : updatedList) {
            // TODO: what is the purpose of this code? It seems to just print "null"
            if (temp == null) {
    ...
    /**
     * Goes through the original text and then makes a recommendation for each character in the given catalog: keep or remove.
     * @param textName
     * @param catalogName
     */
    private static Map<String,String> processFile(String textName, String catalogName) {
        String filePrefix = catalogName.split("\\.")[0];
        String keep = filePrefix + "_SOSkeep.csv";
        String remove = filePrefix + "_SOSremove.csv";
        return processFiles(textName, catalogName, keep, remove);
    }
     ...
     */
    private static Map<String,String> processFiles(String textName, String catalogName,
                                                   String keepCatalogName, String removeCatalogName) {
        List<String> charLines = FileUtils.readFile(catalogName);
        Map<String, String[]> charMap = new HashMap<>();
        Map<String, String> charLineMap = new HashMap<>();
            ...
            for (String key : charMap.keySet()) {
                ...
                if (textLine.contains(tokens[SHORT_CAT_NAME_INDEX])) {
                    ...
                    removalList.add(key);
                        ...
                        && !tokens[SHORT_CAT_ALIAS_INDEX].isEmpty()
                        && textLine.contains(tokens[SHORT_CAT_ALIAS_INDEX])) {
                    System.out.println("matched: " + key + " alias token:" + tokens[SHORT_CAT_ALIAS_INDEX]);
                    removalList.add(key);
                } else if (tokens.length > SHORT_CAT_SURNAME_INDEX
                        && !tokens[SHORT_CAT_TITLE_INDEX].isEmpty()
                        && !tokens[SHORT_CAT_SURNAME_INDEX].isEmpty()) {
                    String titleLastNameToken = tokens[SHORT_CAT_TITLE_INDEX] + ' ' + tokens[SHORT_CAT_SURNAME_INDEX];
                    if (textLine.contains(titleLastNameToken)) {
                        System.out.println("matched: " + key + " title/last name token:" + titleLastNameToken);
                        removalList.add(key);
                    }
                } else if (tokens.length > SHORT_CAT_SURNAME_INDEX
                        && !tokens[SHORT_CAT_FORENAME_INDEX].isEmpty()
                        && !tokens[SHORT_CAT_SURNAME_INDEX].isEmpty()) {
                    String firstLastNameToken = tokens[SHORT_CAT_FORENAME_INDEX] + ' ' + tokens[SHORT_CAT_SURNAME_INDEX];
                    if (textLine.contains(firstLastNameToken)) {
                        System.out.println("matched: " + key + " first/last name token:" + firstLastNameToken);
                        removalList.add(key);
                    }
                }
            }
        ...
        Map<String, String> keepMap = new TreeMap<>();

        List<String> keepLines = new ArrayList<>();
        List<String> removeLines = new ArrayList<>();
...
//    }
    ...
    private static void createAddFile() {
        ...
        for (String previousListFile : previousListFiles) {
            List<String> lineList = FileUtils.readFile(previousListFile);

            for (String line : lineList) {
                map.put(StringUtils.substringBefore(line, ","), line);
            }
        }
        ...
        for (String keepFile : keepFiles) {
            List<String> keepLines = FileUtils.readFile(keepFile);

            for (String keep : keepLines) {
                String key = StringUtils.substringBefore(keep, ",");
                if (map.containsKey(key)) {
                    newLines.add("carryover," + map.get(key));
                } else {
                    newLines.add("keep," + keep);
                }
            }
    ...
    private static void processList(String textName, String names) {
        ...
        for (String w : nameArray) {
            nameList.add(StringUtils.substringBefore(w, "-("));
        }
